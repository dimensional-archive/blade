...
import { Message, Constants, Channel, User } from "eris";
import { DefaultArgumentOptions, TypeResolver, ContextData, Flag, Context } from "../command";
import { StoreOptions, Store, ComponentResolvable } from "./base/Store";
import { InhibitorStore, MonitorStore, SubscriberStore } from ".";
import { Collection, RatelimitManager, Util, Permissions } from "..";
...
export class CommandStore extends Store<Command> {
   ...
   * @readonly
   */
  private readonly promptStorage: Collection<string, Set<string>> = new Collection();
  /**
   * A storage for contexts.
   * @since 1.0.0
   * @readonly
   */
  private readonly contextStorage: Collection<string, Context> = new Collection();
   ...
   * @since 1.0.0
   */
  public add(part: Command): Command | null {
  ...
  public useListenersStore(listenerStore: SubscriberStore): this {
  ...
  public async handle(message: Message): Promise<boolean> {
    try {
      ...
      if (this.contextStorage.has(message.id)) {
        ...
        message.ctx = new (Structures.get("context"))(this, message);
  ...
  public async handleConditionalCommands(message: Message): Promise<boolean> {
    const trueCommands = this.parts.filter(
      (command) =>
