...
export class Storage<K, V> extends Map<K, V> {
   ...
   * @param thisArg Optional binding for the fn param
   */
  public filter(fn: (value: V, key: K, map: this) => boolean, thisArg?: any): Storage<K, V> {
    if (typeof thisArg !== 'undefined') fn = fn.bind(thisArg);

    const results = new (this.constructor as typeof Storage)[Symbol.species]() as Storage<K, V>;
  ...
  public clone(): Storage<K, V> {
    return new (this.constructor as typeof Storage)[Symbol.species](this) as Storage<K, V>;
  }
   ...
   */
  public concat(...Storages: Storage<K, V>[]): Storage<K, V> {
   ...
   */
  public equals(Storage: Storage<K, V>): boolean {
   ...
   */
  public sorted(compareFunction: (v0: V, v1: V, k0?: K, k1?: K) => number = (first, second): number => +(first > second) || +(first === second) - 1): Storage<K, V> {
    const entries = [ ...this.entries() ]
      .sort((e0, e1) => compareFunction(e0[1], e1[1], e0[0], e1[0]));
    return new (this.constructor as typeof Storage)(entries);
  }
}
